...
usage() {
  cat <<'USAGE'
Usage: scripts/verify-local.sh [--live] [--full] [--binary PATH]

Runs the standard local DeepSeek verification suite.
...
section "Local Verification Prompt"
cat <<'PROMPT'
Verify DeepSeek locally after code or dock rendering/navigation changes.
...
section "Dock Cancellation Smoke"
run python3 scripts/phase16-dock-cancel-smoke.py --binary "$BINARY"
